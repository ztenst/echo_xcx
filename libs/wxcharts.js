...
var config = {
    ...
    colors: ['#ff706a', '#4595f8', '#a8cf55', '#90ed7d', '#f15c80', '#8085e9'],
...
function calLegendData(series, opts, config) {
    ...
    var currentRow = [];
    series.forEach(function (item) {
        var itemWidth = 3 * padding + shapeWidth + measureText(item.title || 'undefinded');
...
function drawAreaToolTip(text, offset, opts, config, context) {
    offset = assign({
        x: 0,
        y: 0
    }, offset);
    ...
    var qipaoHeight = 20;
    // var img = getqipao();
    var qipao = '/images/qipao.png';
    context.drawImage(qipao, offset.x - qipaoWidth / 2, offset.y - qipaoHeight, qipaoWidth, qipaoHeight);
    // 绘制数据文案
    context.beginPath();
    context.setFontSize(10);
    context.setFillStyle('#ffffff');
    context.fillText(text, offset.x - measureText(text) / 2, offset.y - qipaoHeight / 2 + 2);
    context.closePath();
    context.stroke();
}
function drawToolTip(textList, offset, opts, config, context) {
    console.log(textList, offset, opts, config, context)
    var legendWidth = 4;
    ...
    var toolTipWidth = legendWidth + legendMarginRight + 16 * config.toolTipPadding + Math.max.apply(null, textWidth);
    ...
    context.beginPath();
    context.setFillStyle(config.toolTipBackground);
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        splitPointList.forEach(function (points) {
            ...
            if (points.length > 1) {
                ...
                points.forEach(function (item, index) {
                    ...
                    // drawAreaToolTip(eachSeries.data[detail-cover], item, opts, config, context);
                });
                points.forEach(function (item, index) {
                    context.moveTo(item.x, startY);
                    context.lineTo(item.x, endY);
                });
        ...
        //drawAreaToolTip and line
        splitPointList.forEach(function (points) {
            context.beginPath();
            context.setStrokeStyle('#d51938');
            context.setLineWidth(1);
            context.moveTo(points[0].x, points[0].y);
            points.forEach(function (item, index) {
                drawAreaToolTip(eachSeries.data[index], item, opts, config, context);
                context.lineTo(item.x, item.y);
            });
            context.stroke();
...
function drawXAxis(categories, opts, config, context) {
    ...
    context.save();
        ...
        categories.forEach(function (item, index) {
            ...
            context.fillText(item, xAxisPoints[index] + offset-4, startY + config.fontSize + 8);
            context.closePath();
            context.stroke();
        });
...
function drawYAxis(series, opts, config, context, offsetX) {
    if (opts.yAxis.disabled === true) {
        return;
    }
    ...
    context.beginPath();
    context.setStrokeStyle(opts.yAxis.gridColor || "#333333");
    context.setLineWidth(1);
    points.forEach(function (item, index) {
...
function drawLegend(series, opts, config, context) {
    ...
    legendList.forEach(function (itemList, listIndex) {
        ...
        itemList.forEach(function (item) {
            ...
            width += 3 * padding + measureText(item.title) + shapeWidth;
        });
        ...
        itemList.forEach(function (item) {
            switch (opts.type) {
                case 'line':
                    ...
                    context.arc(startX + 7.5, startY + 5, 5, 0, 2 * Math.PI);
                    context.fill();
                    context.stroke();
                    context.closePath();
                    break;
            ...
            context.fillText(item.title, startX, startY + 9);
            context.closePath();
            context.stroke();
            startX += measureText(item.title) + 2 * padding;
        });
    });
}
function drawPieDataPoints(series, opts, config, context) {
    ...
    if (opts.type === 'ring') {
        ...
        context.closePath();
        context.fill();
        context.drawImage('/images/iconmoney.png', centerPosition.x - meyiconWidth / 2, centerPosition.y - meyiconHeight / 2, meyiconWidth, meyiconHeight);
    }
